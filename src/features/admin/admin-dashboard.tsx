import { BadgeCheck, Building2, Compass, FlaskConical, Globe2, Hospital, Pill, ScanLine, ShieldAlert, Stethoscope, type LucideIcon } from 'lucide-react';
...
interface DashboardData {
  counts: { hospitals: number; doctors: number; pharmacies: number; radiologyCenters: number; medicalLaboratories: number; specialties: number; countries: number; journeys:number };
  distribution: Record<string, number>;
  journeyBreakdown:Record<string,number>;
  verified: number;
  awaiting: number;
  error: boolean;
}
...
export function AdminDashboard({ copy, data, locale }: { copy: AdminDictionary; data: DashboardData; locale: Locale }) {
  const stats: Array<[string, string, LucideIcon, 'blue' | 'emerald' | 'violet' | 'amber']> = [
    ...
    [copy.dashboard.treatmentJourneys, String(data.counts.journeys), Compass, 'violet'],
  ];
  ...
  return (
    <div className="mx-auto max-w-[92rem]">
      ...
      <div className="mt-7 grid gap-5 xl:grid-cols-3">
        ...
        <Card><CardHeader><h2 className="type-h3 text-[var(--foreground)]">{copy.dashboard.journeyBreakdown}</h2></CardHeader><CardContent className="space-y-4">{[[copy.dashboard.activeJourneys,(data.journeyBreakdown.CONFIRMED??0)+(data.journeyBreakdown.IN_PROGRESS??0)+(data.journeyBreakdown.FOLLOW_UP??0)],[copy.dashboard.planningJourneys,(data.journeyBreakdown.DRAFT??0)+(data.journeyBreakdown.PLANNING??0)+(data.journeyBreakdown.READY_FOR_CONFIRMATION??0)],[copy.dashboard.completedJourneys,data.journeyBreakdown.COMPLETED??0]].map(([label,value])=><div className="flex justify-between text-sm" key={String(label)}><span className="text-[#53697b]">{label}</span><strong>{value}</strong></div>)}</CardContent></Card>
      </div>
    </div>
  );
}
