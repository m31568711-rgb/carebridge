import { BadgeCheck, Building2, Globe2, Hospital, Pill, ShieldAlert, Stethoscope, type LucideIcon } from 'lucide-react';
...
interface DashboardData {
  counts: { hospitals: number; doctors: number; pharmacies: number; specialties: number; countries: number };
...
export function AdminDashboard({ copy, data, locale }: { copy: AdminDictionary; data: DashboardData; locale: Locale }) {
  ...
  const providerMaximum = Math.max(1, data.counts.hospitals, data.counts.doctors, data.counts.pharmacies);
  ...
  return (
    <div className="mx-auto max-w-[92rem]">
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{stats.map(([title, value, icon, tone]) => <StatCard hint={copy.dashboard.description} icon={icon} key={title} title={title} tone={tone} value={value} />)}</div>
      <div className="mt-7 grid gap-5 xl:grid-cols-2">
        <Card><CardHeader><h2 className="type-h3 text-[var(--foreground)]">{copy.dashboard.providersByType}</h2></CardHeader><CardContent className="space-y-5">{[[copy.dashboard.hospitals, data.counts.hospitals], [copy.dashboard.doctors, data.counts.doctors], [copy.dashboard.pharmacies, data.counts.pharmacies]].map(([label, value]) => <div key={String(label)}><div className="flex justify-between text-sm"><span className="text-[#53697b]">{label}</span><strong className="text-[var(--foreground)]">{value}</strong></div><div className="mt-2 h-2 rounded-full bg-[#edf3f7]"><div className="h-full rounded-full bg-[var(--primary)]" style={{ width: `${Math.max(4, Number(value) / providerMaximum * 100)}%` }} /></div></div>)}</CardContent></Card>
