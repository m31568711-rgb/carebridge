...
export interface InvoiceRecord{id:string;invoice_number:string;booking_id:string;currency:string;subtotal:number;total_amount:number;amount_paid:number;due_date:string|null;status:string;notes:string|null;issued_at:string|null;created_at:string;items?:Array<{id:string;description:string;quantity:number;unit_amount:number;base_unit_amount?:number;carebridge_fee_percent?:number;carebridge_fee_amount?:number;line_amount:number}>;payments?:Array<{id:string;amount:number;paid_at:string;method:string;reference_number:string|null}>}
