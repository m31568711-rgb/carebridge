...
export interface ProgramService{id:string;service_type:string;status:string;title:string;notes:string|null;planned_date:string|null;treatment_id:string|null;hospital_id:string|null;appointment_id:string|null;treatment:{name_i18n:Record<string,string>}|null;hospital:{name_i18n:Record<string,string>}|null;appointment:{scheduled_at:string;status:string;location_name:string;instructions:string|null}|null}
...
export async function loadDoctorProgram(s:SupabaseClient,bookingId:string,locale:Locale):Promise<DoctorProgramData>{
 ...
 const[treatmentRows,hospitalRows,services]=await Promise.all([treatments,s.from('hospitals').select('id,name_i18n').eq('status','ACTIVE').eq('is_verified',true).order('legal_name').limit(300),s.from('journey_services').select('id,service_type,status,title,notes,planned_date,treatment_id,hospital_id,appointment_id,treatment:treatments(name_i18n),hospital:hospitals(name_i18n),appointment:appointments(scheduled_at,status,location_name,instructions)').eq('booking_id',bookingId).in('service_type',['DOCTOR_CONSULTATION','HOSPITAL_PROCEDURE']).order('created_at')]);
 return{treatments:(treatmentRows.data??[]).map(x=>({id:x.id,label:localized(x.name_i18n,locale)})),hospitals:(hospitalRows.data??[]).map(x=>({id:x.id,label:localized(x.name_i18n,locale)})),services:(services.data??[])as unknown as ProgramService[]};
}
